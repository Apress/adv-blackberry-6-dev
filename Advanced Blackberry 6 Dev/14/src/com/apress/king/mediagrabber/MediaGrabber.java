        ...
        MediaGrabberResource, PushApplication, ShortcutEventListener
{
    ...
    public static final String DATABASE = "com.apress.king.MediaGrabber";
    ...
    {
        /*
         * try { Scratch.doDB(); } catch (Exception e) { e.printStackTrace(); }
         */
            ...
            try
            {
                ...
                if (store.get(menuItemID) == null)
                {
                    ...
                    ApplicationDescriptor pushDescriptor = new ApplicationDescriptor(
                            ...
                            { "push" });
                    PushApplicationDescriptor registeredDescriptor = new PushApplicationDescriptor(
                            4242, pushDescriptor);
    ...
    static private void initDatabase()
    {
        try
        {
            if (!DatabaseFactory.exists(DATABASE))
            {
                Database db = DatabaseFactory.create(DATABASE);
                Statement creation = db.createStatement
                    ("CREATE TABLE 'Playlist' ('Path' TEXT, 'Timestamp' INTEGER)");
                creation.prepare();
                creation.execute();
                db.close();
            }
