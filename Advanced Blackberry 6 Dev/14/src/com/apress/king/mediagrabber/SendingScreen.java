...
private ActivityIndicatorModel sendingModel;
    ...
    private MenuItem selectItem = new MenuItem(r,
            I18N_COMMAND_CHOOSE_CONTACT, 0, 0)
    {
        public void run()
        {
            selectRecipient();
        ...
        byte[] data, boolean encrypt)
{
    ...
    status = new LabelField(r, I18N_PROMPT_EMAIL);
    receiver = new BasicEditField(r.getString(I18N_LABEL_RECIPIENT), "",
            100, BasicEditField.FILTER_EMAIL | Field.USE_ALL_WIDTH);
    ActivityIndicatorView busy = new ActivityIndicatorView(0);
    Bitmap bitmap = Bitmap.getBitmapResource("waiting.png");
    busy.createActivityImageField(bitmap, 12, 0);
    sendingModel = new ActivityIndicatorModel();
    busy.setModel(sendingModel);
    sendingModel.cancel();
    add(busy);
    add(status);
    add(receiver);
    ...
    private void send()
    {
        status.setText(r, I18N_STATUS_SENDING);
        state = STATE_SENDING;
